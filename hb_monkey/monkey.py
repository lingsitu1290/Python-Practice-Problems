...
def earliest_arrival(jump_distance, stones):
    ...
    # while the monkey's jumping power is less than the length of the stones
    # meaning the monkey can't cross
    while position + jump_distance < len(stones):

        # Position of rocks where the monkey can jump to 
        possible_landings = stones[position + 1 : position + jump_distance + 1]
        print possible_landings

        # Given the possible positions of where the monkey can jump, get the minimum number(the earliest time the monkey can jump)
        soonest_landing = min(possible_landings)
        landings.append(soonest_landing)

        print landings
        # Find the index of the earliest time 
        position = stones.index(soonest_landing)
